...
**The boundary that matters:** `internal/tui` and `internal/cli` must only import `internal/core/*` (and the dependency-free `internal/buildinfo`/`internal/logo`) — never `internal/imap`, `internal/smtp`, `internal/storage`, or `internal/sync` directly, and never each other (`internal/cli`'s import of `internal/tui` to launch the TUI from the root command is the one sanctioned exception). Those lower-level packages are consumed exclusively by `internal/core/*`. This is what lets `internal/httpapi` be added later as a third frontend without touching business logic. When adding a feature, put the actual logic in the relevant `internal/core/<domain>` service and keep `internal/tui`/`internal/cli` code to rendering/argument-parsing plus calls into that service. Static, cross-frontend presentation data (e.g. the pigeon logo) belongs in its own small dependency-free package like `internal/logo`, not in one frontend for the other to import.

**Credentials vs. config:** account connection settings (host, port, TLS, username) live in a TOML config file (`internal/config`); secrets (passwords, tokens) live in the OS keyring via `internal/auth`, never in the config file or the sqlite DB. `internal/auth` defines a `Provider` interface (`IMAPSASLClient`/`SMTPSASLClient`) with two implementations selected by `Account.AuthType` via `auth.NewProvider`: `PasswordProvider` (SASL PLAIN, password from the keyring) and `GoogleProvider` (a hand-rolled XOAUTH2 `sasl.Client` — go-sasl only ships OAUTHBEARER — backed by an OAuth2 token from the keyring, auto-refreshed and written back on refresh). `auth.LoginGoogle` runs Google's interactive "installed app" loopback flow (local HTTP server + PKCE, no client-secret confidentiality assumed) for `pigeon account add --google` / `pigeon account edit --reauth-google` and the TUI's add-account form. The Google OAuth client ID/secret are baked into `internal/auth/google.go` as constants (placeholder values — see the `TODO` there) rather than pulled from user config, following the same "installed app secret isn't actually secret" model Thunderbird/rclone use; `PIGEON_GOOGLE_OAUTH_CLIENT_ID`/`_SECRET` env vars override them for testing against a different Google Cloud project without a rebuild. Every `internal/core/*` call site that dials IMAP/SMTP builds its provider via `auth.NewProvider(cfg.AuthType, cfg.Username, cfg.Slug)` rather than constructing a provider type directly, so a third auth type only means adding a case there.
...
**Logging:** `internal/logging` installs a process-wide `log/slog` default logger (`slog.SetDefault`) writing to a file — never stdout/stderr, since the TUI takes over the full terminal and any stray print would corrupt the rendered frame. `internal/cli/root.go`'s `PersistentPreRunE`/`PersistentPostRun` initialize/close it for every subcommand, controlled by `--log-level` (debug/info/warn/error) and `--log-file` (default `$XDG_CACHE_HOME/pigeon/pigeon.jsonl`, written as JSON Lines). Call sites use the package-level `slog.Info`/`slog.Debug`/etc. directly (no logger threaded through `internal/core`/`internal/sync`/`internal/imap` signatures) — it's additive to, not a replacement for, the TUI's ephemeral status bar (`internal/tui/status.go`) and the CLI's existing user-facing stdout output.
